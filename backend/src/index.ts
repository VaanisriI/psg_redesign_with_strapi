...
export default {
  ...
  async bootstrap({ strapi }) {
    ...
    const hasPresent =
      Array.isArray((incubateeProfileExists as any)?.present_incubatees) &&
      (incubateeProfileExists as any).present_incubatees.length > 0;

    console.log('[Seed] incubatee-profile doc:', incubateeProfileExists?.documentId ?? 'none', '| hasPresent:', hasPresent);
    ...
    if (!hasPresent) {
      ...
      if (incubateeProfileExists?.documentId) {
